...
class Trainer:
    def __init__(self, cfg: ExperimentConfig, dist: DistributedContext) -> None:
        self.cfg = cfg
        self.dist = dist
        ...
        self.backend = QwenTTSBackend(cfg.model)
        self.experts = ExpertSuite.load(cfg.model.device)
    ...
    def train(self, cadence_eval=None) -> None:
        while self.completed < self.cfg.train.max_updates:
            self.run_update()
            if cadence_eval is not None and \
        ...
        for name, module in frozen.items():
            ...
            for p in module.parameters():
                if p.grad is not None:
                    raise RuntimeError(f"frozen model {name!r} received a gradient")
                if p.requires_grad:
                    raise RuntimeError(f"frozen model {name!r} has requires_grad=True")
                break  # first parameter suffices as a tripwire per update
